...
<head>
	...
	function addETeilRow(tableId) {
		var TR_E, TR_E_Clone;
		TR_E = document.getElementById("TR_E");
		TR_E_Clone = TR_E.cloneNode(true);
		
		//set inputs of clone back to zero
		var ausgabe = "";
		var count=1;
		
		for (var i = 0; i <= TR_E_Clone.childNodes.length-1; i++) {
			ausgabe = ausgabe + "name: " + TR_E_Clone.childNodes[i].nodeName + "; value: " + TR_E_Clone.childNodes[i].nodeValue + "\n";
			
			if (TR_E_Clone.childNodes[i].nodeName == "TD") {
				//set the second input-field (3rd <td>-tag) back to zero
				//alert("count: " + count)
				if (count < 3) {
					count++;
				}
					...
					//alert("in");
					//alert("name: " + TR_E_Clone.childNodes[i].nodeName + "; value: " + TR_E_Clone.childNodes[i].nodeValue);
					TR_E_Clone.childNodes[i].firstChild.value = 0;					
				}
			}
		}	
		//alert(ausgabe);
...
'response.write "http-get 'd': " & d & "<br>"
'response.write "vartype(d): " & d & "<br>"

if d=vbEmpty or d<1 or d>5 then
	response.write "ERROR. Aktueller Tag konnte nicht korrekt aus HTTP-Get ermittelt werden.<br><br>"
	d="void"
end if

'continue only if reading of http-get was succesful
if d<>"void" then
...
<input type="hidden" name="formularType" value="production">
<% 
	response.write "<input type=""hidden"" name=""d"" value="&d&">"
'response.write "<input type='hidden' name='day' value='"&d&"'>" 
%>
...
'print input fields for new tasks
'*********************************		
...
for each i in idArrayE
		...
		if i <> vbEmpty then
			...
			response.write "<tr>"
			response.write "<td align='center'>"
			response.write "<form method=""POST"" action='erpFormularProduction.asp?d=" &d& "' style='margin-bottom: 0;'>"
...
end if 'http-get ERROR (if d<>"void")
%>
</div> <!-- container -->
</body>
</html>